...
final class DecodeHandler extends Handler {
    ...
    public void handleMessage(Message message) {
        if( message.what == e.jesslyn.win_2.R.id.decode)
        {
            decode((byte[]) message.obj, message.arg1, message.arg2);
        }else if( message.what == e.jesslyn.win_2.R.id.quit)
        {
    ...
    private void decode(byte[] data, int width, int height) {
        ...
        if (rawResult != null) {
            Message message = Message.obtain(mActivity.getCaptureActivityHandler(), e.jesslyn.win_2.R.id.decode_succeeded, rawResult);
            message.sendToTarget();
        } else {
            Message message = Message.obtain(mActivity.getCaptureActivityHandler(), e.jesslyn.win_2.R.id.decode_failed);
            message.sendToTarget();
        }
    }
}
